...
## 0. /nh:make 2026-04-23 Boss Transition Implementation
- Scope:
  - Lock the boss-clear transition earlier and preserve a single verified stage-advance path.
  - Clean up the live implementation so boss defeat branches reuse one transition helper.
- Implemented:
  - Added `stageAdvancePending` and blocked damage during pending boss-clear transitions.
  - Routed `dragonBoss`, `nightmareBoss`, and non-stage1 `boss` clears through `transitionAfterBossDefeat()`.
  - Restored the malformed `START MISSION` button config block in `public/games/ninja/script.js`.
  - Kept the legacy transition block non-executable so the active path remains `goToNextStage() -> stageAdvanceTicket -> startNextStageScene()`.
- Verification:
