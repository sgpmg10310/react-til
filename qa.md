...
## 학습 자료 5종 QA (2026-09-25)
- 기준: 5개 페이지가 메뉴·허브·홈에서 열리고, 모든 데모·연습장·퀴즈가 실제로 동작하며, 폰 폭에서 레이아웃이 깨지지 않는다.
- 자동 테스트:
  - `runCode.test.js` (7): 출력 모으기, scope 값 넣기, 실행 에러·문법 에러, strict 모드, 값 문자열로 바꾸기, 순환 참조.
  - `studyLogic.test.js` (8): 영향 받는 패키지(utils/api-client/web), 빌드 순서가 의존성 먼저, 대상만 포함, 토큰 자르기(키워드·숫자·문자열·=>·unknown).
- Playwright 확인: Immer 데모 1~3 + 연습장(원본 안 바뀜, freeze), Swift nil 강제 꺼내기 크래시 표시, RN 기본 column → row/space-between 적용, 모노레포 utils 수정 시 6개 / web 수정 시 1개 빌드(76% 절약), SWC es5+classic 출력, AST 도전 과제 풀면 결과 7.
- 결과: 테스트·lint(새 파일)·build·브라우저 확인 **PASS**.
- 남은 위험:
  - 연습장은 메인 스레드에서 실행하므로 무한 반복문을 쓰면 탭이 멈춤 (학습용이라 사용자 본인 코드만 실행, 보안 영향 없음).
  - Swift·React Native는 브라우저에서 진짜로 실행할 수 없어 시뮬레이터·퀴즈·JS 번역으로 대신함.
  - SWC 변환 결과는 SWC 출력 형태를 정리한 고정 예시(실시간 SWC 실행 아님).
